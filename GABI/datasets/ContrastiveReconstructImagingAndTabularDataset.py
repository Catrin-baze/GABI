  ...
  def __init__(
    ...
    if augmentation_speedup:
      ...
      A.Lambda(name='convert2tensor', image=convert_to_ts_01)
        ...
        self.default_transform = A.Compose([
      A.Resize(height=img_size, width=img_size),
      A.Lambda(name='convert2tensor', image=convert_to_ts_01)  # ADNI 通常使用 0-255 float，无需归一化
      ])
        print(f'Using adni transform for default transform in ContrastiveReconstructImagingAndTabularDataset')
      else:
        raise ValueError('Only support dvm, cardiac, and adni datasets')
    else:
...
if __name__ == '__main__':
    dataset = ContrastiveReconstructImagingAndTabularDataset(
        data_path_imaging='/home/ZhangYinuo/MRI/Data/ADNI/features/adni_train_paths_imaging.pt', 
        ...
        data_path_tabular='/home/ZhangYinuo/MRI/Data/ADNI/features/adni_features_train_imputed_noOH_tabular_imaging_reordered.csv',
        corruption_rate=0.15, 
        replace_random_rate=0.1, 
        replace_special_rate=0.4, 
        field_lengths_tabular='/home/ZhangYinuo/MRI/Data/ADNI/features/tabular_lengths_reordered.pt',
        one_hot_tabular=False,
        labels_path='/home/ZhangYinuo/MRI/Data/ADNI/features/adni_labels_train.pt',
        img_size=224,
        live_loading=True,
        augmentation_speedup=False
    )
    ...
    if isinstance(x, (tuple, list)):
        for i, item in enumerate(x):
            try:
                shape_or_len = getattr(item, 'shape', len(item) if hasattr(item, '__len__') else 'N/A')
            except TypeError:
                shape_or_len = 'scalar (0-dim)'
            print(f"Item {i}: Type: {type(item)}, Shape/Length: {shape_or_len}")
    else:
        print(f"Output Type: {type(x)}, Shape/Length: {getattr(x, 'shape', len(x) if hasattr(x, '__len__') else 'N/A')}")
